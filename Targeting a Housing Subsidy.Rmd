...
```{r exploratory_binary, message = FALSE, warning = FALSE}
H_subsidy %>%
    dplyr::select(y, contact, campaign, poutcome) %>%
    gather(Variable, value, -y) %>%
    count(Variable, value, y) %>%
      ggplot(., aes(value, n, fill = y)) +   
        geom_bar(position = "dodge", stat="identity") +
...
We will conduct a series of *stepwise regressions* in order to identify which variables are best for our model. Stepwise regression is a predictive procedure wherein predictors (variables) are repeatedly added/removed in order to find the collection of variables in a dataset that result in the best performing model, (a model that lowers prediction error).
...
#H_subsidy <- 
  ...
  #right_join(H_subsidy, .) %>%
  mutate(job_cat = job == "admin."  ~ "admin.",
...
```{r create_partition}
set.seed(3456)
trainIndex <- createDataPartition(H_subsidy$y, p = .65,
                                  list = FALSE,
                                  times = 1)
...
``` {r fit metrics}
results <- results %>% 
        rename("Model 1" = "pR2.H_subsidyModel_1.",
               "Model 2." = "pR2.H_subsidyModel_2.",
               "Model 3" = "pR2.H_subsidyModel_3.")
